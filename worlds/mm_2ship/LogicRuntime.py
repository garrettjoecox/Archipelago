...
ports the C++ solver semantics (FindReachableRegions, TimeLogic expansion, the
...
Data flows in from the 2ship sources on its own. The region graph, conditions,
helper predicates, item grant maps and enum values all live in the generated
modules, so this file only needs attention when the C++ *algorithms* change.
The genlogic drift ledger flags exactly that.
...
from .OptionData import RO_CHOICE_VALUES, RO_OPTIONS
...
def _item(key: str) -> str:
    """AP display name for an RI enum key. Reading it out of ItemData means an
    upstream display-name change flows through instead of quietly breaking a
    hard-coded string here."""
    return ITEMS[key].name


# Option value constants used structurally here (rather than through a
# translated condition, which would have emitted them into LogicHelpersGen).
# Read from the generated Types.h mirror rather than hand-transcribed: these
# choice enums do get members inserted and reordered upstream, and a stale
# literal here would silently mean "no option ever matches".
RO_CLOCK_SHUFFLE_RANDOM = RO_CHOICE_VALUES["RO_CLOCK_SHUFFLE_RANDOM"]
RO_CLOCK_SHUFFLE_ASCENDING = RO_CHOICE_VALUES["RO_CLOCK_SHUFFLE_ASCENDING"]
RO_CLOCK_SHUFFLE_DESCENDING = RO_CHOICE_VALUES["RO_CLOCK_SHUFFLE_DESCENDING"]

RO_DUNGEON_ITEM_START_WITH = RO_CHOICE_VALUES["RO_DUNGEON_ITEM_START_WITH"]

# z64save.h: stray fairies scattered through one dungeon
STRAY_FAIRY_SCATTERED_TOTAL = 15

RO_ACCESS_DUNGEONS_FORM_AND_SONG = RO_CHOICE_VALUES["RO_ACCESS_DUNGEONS_FORM_AND_SONG"]
RO_ACCESS_DUNGEONS_FORM_OR_SONG = RO_CHOICE_VALUES["RO_ACCESS_DUNGEONS_FORM_OR_SONG"]
RO_ACCESS_DUNGEONS_FORM_ONLY = RO_CHOICE_VALUES["RO_ACCESS_DUNGEONS_FORM_ONLY"]
RO_ACCESS_DUNGEONS_SONG_ONLY = RO_CHOICE_VALUES["RO_ACCESS_DUNGEONS_SONG_ONLY"]
RO_ACCESS_DUNGEONS_OPEN = RO_CHOICE_VALUES["RO_ACCESS_DUNGEONS_OPEN"]
...
class LogicContext:
    ...
    def has_item(self, item_const: str) -> bool:
        ...
        AND bombchu slots, while a refill pack that can't raise its own
        capacity grants nothing. See genlogic's pack_normalize and
        extra_slot_grants."""
...
class Solver:
    ...
    def __init__(self, world: "MM2ShipWorld"):
        ...
        # Checks disabled by options are not AP locations, but they still hand
        # out their vanilla items in-game when reached: an unshuffled shop still
        # sells the All-Night Mask. GlitchlessLogic.cpp models that by
        # GiveItem()ing every reachable check outside the shuffle pool, so the
        # solver self-grants those vanilla items during the fixpoint.
        self.disabled_check_vanilla: dict[str, str] = self._compute_disabled_vanilla(world)

        # Every AP item name the logic can observe. Items outside this set
        # (triforce pieces, filler, traps) can never change a solve result,
        # so collect/remove skip cache invalidation for them and result_for
        # filters them out of the memo signature.
        self.logic_item_names: frozenset[str] = self._compute_logic_item_names()

        # world-level memo: inventory signature -> SolveResult
        self._memo: dict[tuple, SolveResult] = {}

    def _compute_logic_item_names(self) -> frozenset[str]:
        names: set[str] = set()
        for table in (ITEM_TO_ITEMS, RANDO_INF_TO_ITEMS, WEEKEVENTREG_TO_ITEMS,
                      QUEST_TO_ITEMS, TOKEN_SCENE_TO_ITEMS, OWL_WARP_TO_ITEMS):
            for grants in table.values():
                names.update(grants)
        names.update(ACTOR_TO_SOUL_ITEM.values())
        names.update(BOTTLE_ITEMS)
        names.update(MOON_MASK_ITEMS)
        names.update(REMAINS_ITEMS)
        for info in DUNGEON_ITEMS.values():
            names.update(info.values())
        names.update(self.clock_items)
        ...
        names.update(self.disabled_check_vanilla.values())
    ...
    #    GetComputedStartingItems, plus what GrantStartingItems adds) ---------
    ...
    def _compute_starting_items(self) -> dict[str, int]:
        ...
        if opts.get("RO_PLACEMENT_STRAY_FAIRIES") == RO_DUNGEON_ITEM_START_WITH:
            ...
            # Belongs to no dungeon, but Start With hands it over too, and it
            # matters to logic: it sets WEEKEVENTREG_08_80, which is the Clock
            # Town Great Fairy's reward check.
            give("CLOCK_TOWN_STRAY_FAIRY")
        ...
        if opts.get("RO_STARTING_BUNNY_HOOD"):
            give("MASK_BUNNY")
        # GrantStartingItems hands out full consumables on top of whatever
        # GetComputedStartingItems returns, and it runs before the C++ solver
        # (OnFileCreate.cpp grants, then applies logic) and again on AP connect,
        # so logic has to assume them. Deku Sticks and Nuts gate torches and a
        # long list of CanKillEnemy rules. Its other two grants need nothing
        # here: starting health arrives through self.starting_health, and
        # starting rupees never matter because CAN_AFFORD reads only the wallet
        # tier.
        if opts.get("RO_STARTING_CONSUMABLES"):
            give("DEKU_STICK")
            give("DEKU_NUT")
        # Clock shuffle's guaranteed starting time item arrives through
        # state.prog_items instead: MM2ShipWorld precollects it as a real AP
        # item so that the client grants it too.
        return start
    ...
    def result_for(self, state) -> SolveResult:
        ...
        if res is None:
            ...
            relevant = self.logic_item_names
