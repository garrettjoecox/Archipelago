...
def parse_menu_sliders(menu_cpp: str, resolve_constant) -> dict[str, tuple[int | None, int | None, int | None]]:
    ...
    A None field means the value is dynamic in the UI — e.g. the Required
    sliders' Max reads the paired _MAX option's current value, so their static
    ceiling is the paired option's Max instead.
    """
    ...
    for m in re.finditer(r"CVarSliderInt\s*\(", menu_cpp):
        ...
        def field(name: str) -> int | None:
            ...
            if "(" in arg:  # runtime expression (CVarGetInteger(...)) — dynamic
                return None
...
def main() -> None:
    ...
    # these ordinals exactly — slot_data sends the raw int to the game.
