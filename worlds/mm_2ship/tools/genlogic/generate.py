...
def parse_menu_sliders(menu_cpp: str, resolve_constant) -> dict[str, tuple[int | None, int | None, int | None]]:
    ...
    These are the authoritative UI bounds for every numeric option; the AP
    Options.py Range classes must mirror them (enforced by the world's tests).
    A None field means the UI computes that value at runtime. The Required
    sliders' Max, for instance, reads the paired _MAX option's current value,
    so their static ceiling is that paired option's Max.
    """
    out: dict[str, tuple[int | None, int | None, int | None]] = {}
    for m in re.finditer(r"CVarSliderInt\s*\(", menu_cpp):
        open_pos = m.end() - 1
        end = cpp.find_balanced(menu_cpp, open_pos, "(", ")")
        call = menu_cpp[open_pos:end]
        ro_m = re.search(r"Options\[(RO_\w+)\]", call)
        if not ro_m:
            continue

        def field(name: str) -> int | None:
            fm = re.search(rf"\.{name}\s*\(", call)
            if not fm:
                return None
            fend = cpp.find_balanced(call, fm.end() - 1, "(", ")")
            arg = call[fm.end():fend - 1].strip()
            if "(" in arg:  # runtime expression, e.g. CVarGetInteger(...)
                return None
            return resolve_constant(arg)

        out[ro_m.group(1)] = (field("Min"), field("Max"), field("DefaultValue"))
    return out
...
def compute_quest_item_grants(enums: dict[str, list[tuple[str, int]]],
    ...
    Tolerates name divergence like QUEST_SONG_BOSSA_NOVA <-> ITEM_SONG_NOVA."""
...
def parse_case_value_map(cpp_src: str, func_re: str, case_re: str, value_re: str,
                         what: str) -> dict[str, str]:
    """`case A: case B: <value line>` in one C++ function -> {A: value, B: value}.

    Every caller feeds a table the apworld cannot do without, and a switch that
    parses to nothing looks exactly like a switch that legitimately has no rows,
    so an empty result is a hard failure rather than an empty table. That is not
    hypothetical: upstream renamed DungeonItemToDungeon out from under this and
    left a same-signature function behind, which parsed clean and silently
    declassified every key and fairy as non-progression.
    """
    body = cpp.extract_function_body(cpp_src, func_re)
    out: dict[str, str] = {}
    pending: list[str] = []
    for line in body.splitlines():
        line = line.strip()
        cm = re.match(case_re, line)
        if cm:
            pending.append(cm.group(1))
            continue
        vm = re.match(value_re, line)
        if vm:
            for label in pending:
                out[label] = vm.group(1)
            pending = []
    if not out:
        sys.exit(f"ERROR: {what} mapped nothing. Did PlacementConstraints.cpp rename "
                 f"the function or restructure the switch?")
    return out


def parse_scene_to_dungeon(placement_cpp: str) -> dict[str, str]:
    """SceneIdToDungeon(): SCENE_* -> DUNGEON_SCENE_INDEX_*."""
    return parse_case_value_map(
        placement_cpp, r"int\s+SceneIdToDungeon\s*\(",
        r"^case\s+(SCENE_\w+)\s*:$", r"^return\s+(DUNGEON_SCENE_INDEX_\w+)\s*;$",
        "SceneIdToDungeon()")


def parse_item_to_dungeon(placement_cpp: str) -> dict[str, str]:
    """DungeonItemToDungeon(): RI_* -> DUNGEON_SCENE_INDEX_* (option-independent part).

    This is the whole dungeon-item family: every small key, boss key and stray
    fairy that belongs to one dungeon. It feeds both LogicHelpersGen's
    DUNGEON_ITEMS and compute_progression's "keys" family.
    """
    return parse_case_value_map(
        placement_cpp, r"int\s+DungeonItemToDungeon\s*\(",
        r"^case\s+(RI_\w+)\s*:$", r"^dungeon\s*=\s*(DUNGEON_SCENE_INDEX_\w+)\s*;$",
        "DungeonItemToDungeon()")
...
DRIFT_TARGETS = [
    ...
    # Normalizes options before generation (e.g. stray_fairies_max under vanilla
    # placement). MM2ShipWorld.generate_early hand-ports the normalizations that
    # matter to AP; deliberately NOT mirrored is the start_with
    # stray_fairies_required forcing, which is inert here (the player owns all 15).
    ("OnFileCreate.cpp",         "mm/2s2h/Rando/MiscBehavior/OnFileCreate.cpp", None),
...
def read_build_version(root: Path) -> str:
    """The 2ship project version, which the client reports as gBuildVersion.
...
def main() -> None:
    ...
    # Global constant table; last-wins, since names are unique in practice.
    # Plain integer #defines belong in it too, because option defaults are often
    # written as macros: STRAY_FAIRY_SCATTERED_TOTAL from Logic.h, or
    # SPIDER_HOUSE_TOKENS_REQUIRED out of the En_Sth actor header that
    ...
    unknown_slider_ros = sorted(set(slider_ranges) - set(options))
    ...
    # RandoClockShuffleOptions, ...). The AP Choice options' values must match
    # these ordinals exactly, since slot_data sends the raw int to the game.
    # RandoOptionId is the option-index enum, not a choice enum; skip it.
    ro_choice_values: dict[str, int] = {}
    for enum_name, members in enums.items():
        if enum_name == "RandoOptionId" or not members:
            continue
        if all(member.startswith("RO_") for member, _ in members):
            for member, value in members:
                ro_choice_values[member] = value
    ...
    # once: nothing in Items.cpp carries it as an itemId, so it only arrives
    # through extra_slot_grants off the Bomb Bags.
    ungrantable = sorted(
    ...
    if ungrantable:
        print(f"[genlogic] WARNING: {len(ungrantable)} item(s) tested by the logic have no "
              f"granting item in Items.cpp, so those conditions are never satisfiable:")
...
def compute_progression(*, usages: set[str], items: dict[str, dict],
    ...
    """The RI_* ids whose items can gate logic.

    An item is progression when it grants something the translated rules can
    test: an inventory item, a rando-inf flag, a week event reg, a quest item.
    The `families` table below adds the item classes that are always worth
    treating as logic-relevant even when no rule names them directly.
    """
    prog: set[str] = set()
