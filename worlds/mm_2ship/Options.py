...
# (StaticData/Options.cpp + Menu.cpp). slot_data sends the raw values straight
# to the game, so numeric bounds, defaults and choice ordinals have to match
# the C++ side; test/test_options.py enforces that against the generated
# OptionData tables, and documents the deliberate divergences.
# ---------------------------------------------------------------------------
...
class Logic(Choice):
    ...
    A seed generated this way may be impossible to complete without cheating,
    so the host has to allow it first (allow_true_no_logic under
    mm_2ship_options in host.yaml).
    """
    display_name = "Logic"
...
class AccessTrials(Choice):
    ...
    Vanilla: masks are handed over as in the original game, so 1, 2, 3 then 4
    regular masks to get in.
    ...
    Forms: each trial requires its matching transformation mask, and the Link
    Trial is always open.
    ...
    option_forms = 3
    option_open = 4
    default = 0  # RO_ACCESS_TRIALS_VANILLA
...
class HintsMoonGossipStones(Toggle):
    """
    Each Gossip Stone inside the Moon Trials hints where the mask it describes
    in the vanilla game can be found.
    """
    display_name = "Hints: Moon Gossip Stones"
    default = 0
...
class PlacementSmallKeys(Choice):
    ...
    Vanilla: Small Keys stay at the locations that hold them in the vanilla
    game and none are added to the item pool.
    """
...
class PlacementBossKeys(Choice):
    ...
    the item pool.
    Vanilla: each Boss Key stays at the location that holds it in the vanilla
    game and none are added to the item pool.
    """
...
class PlacementStrayFairies(Choice):
    ...
    players' worlds. The Clock Town Stray Fairy joins the item pool with them.
    Own Dungeon: each dungeon's Stray Fairies are confined to locations within
    that same dungeon. The Clock Town Stray Fairy stays where it is in the
    vanilla game, since it belongs to no dungeon.
    Start With: you begin with every Stray Fairy, the Clock Town one included,
    and none are added to the item pool, so Stray Fairies in Pool has no effect.
    Vanilla: Stray Fairies stay at the locations that hold them in the vanilla
    game and none are added to the item pool, so Stray Fairies in Pool has no
    effect and all 15 per dungeon are always obtainable.
    """
...
class ShuffleWonderItems(Toggle):
    ...
    That means invisible rupees and the item clusters that appear when you
    strike an unmarked trigger, such as the Termina Field walls and the
    spider houses.
    """
...
class TriforcePiecesMax(Range):
    ...
    Hunt is on. Asking for more pieces than there are free locations clamps
    the count down; enable more shuffle options to make room for all of them.
    """
...
class StrayFairiesMax(Range):
    """
    Maximum Stray Fairies per dungeon that can appear in the item pool. Has no
    effect when Placement: Stray Fairies is Start With or Vanilla, which put
...
class StrayFairiesRequired(Range):
    ...
    in Pool are clamped down to it, except under Placement: Stray Fairies
    Vanilla, where all 15 per dungeon sit at their vanilla locations.
    """
...
class MM2ShipOptions(PerGameCommonOptions):
    ...
    hints_moon_gossip_stones: HintsMoonGossipStones
