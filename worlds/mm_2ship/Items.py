...
# Count-based items (triforce pieces, tokens, fairies) are individually
# insignificant and plentiful: skip_balancing keeps progression balancing from
# dragging hundreds of them around, deprioritized keeps them off player-chosen
# priority locations.
_CLASSIFICATION_OVERRIDES: dict[str, IC] = {
    "TRAP": IC.trap,
    "TRIFORCE_PIECE": IC.progression_deprioritized_skip_balancing,
    "GS_TOKEN_SWAMP": IC.progression_deprioritized_skip_balancing,
    "GS_TOKEN_OCEAN": IC.progression_deprioritized_skip_balancing,
    "WOODFALL_STRAY_FAIRY": IC.progression_deprioritized_skip_balancing,
    "SNOWHEAD_STRAY_FAIRY": IC.progression_deprioritized_skip_balancing,
    "GREAT_BAY_STRAY_FAIRY": IC.progression_deprioritized_skip_balancing,
    "STONE_TOWER_STRAY_FAIRY": IC.progression_deprioritized_skip_balancing,
}
...
def _classify(key: str, entry) -> IC:
    ...
    if entry.progression:
        ...
        # pieces, ...): plentiful and individually insignificant too.
        if entry.ritype in ("RITYPE_JUNK", "RITYPE_HEALTH"):
            return IC.progression_deprioritized_skip_balancing
...
# Mirrors the generated ItemData: ids are permanent, and names match
# StaticData/Items.cpp because the game client resolves items by name.
