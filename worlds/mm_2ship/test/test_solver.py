...
These guard the pipeline end to end. If a regenerated RegionData or
LogicHelpersGen breaks coverage, or the vanilla layout stops being completable,
they fail before any of it reaches a real multiworld.
"""
...
class TestSolverCoverage(MM2ShipTestBase):
    ...
    def test_non_progression_items_cannot_gate_logic(self) -> None:
        ...
        classifies as filler, useful or trap may open a check. Those items never
        enter CollectionState.prog_items, so one that gated logic would leave
        fill free to hide progression behind a check the player cannot reach.
        The bombchu refill packs did exactly that."""
        solver = self.world.logic
    ...
    def test_explosives_require_a_bomb_bag(self) -> None:
        """CAN_USE_EXPLOSIVE has to track what the player can actually detonate.
        Item_Give's bomb and bombchu refill branches fill the inventory slot but
        clamp ammo to CUR_CAPACITY(UPG_BOMB_BAG), which is 0 without a bag, so a
        pack on its own is an icon with no ammo."""
        solver = self.world.logic
        ...
        # The Goron Mask is what makes this assertion bite: the dropped clause
        # needed it, so a keg alone could never have satisfied it anyway.
...
class TestStartingConsumables(MM2ShipTestBase):
    ...
    def test_starting_consumables_are_in_logic(self) -> None:
        ...
        (OnFileCreate.cpp) and again on AP connect, so the solver has to assume
        them. ITEM_DEKU_STICK gates torch lighting and a long list of
        CanKillEnemy rules."""
        solver = self.world.logic
        ctx = LogicContext(solver, dict(solver.starting_counts))
...
class TestSolveCacheScope(MM2ShipTestBase):
    ...
    def test_goal_items_do_not_invalidate_cache(self) -> None:
        """Triforce pieces are goal-only, so collecting one must not throw away
        the cached solve. A logic-relevant item must."""
